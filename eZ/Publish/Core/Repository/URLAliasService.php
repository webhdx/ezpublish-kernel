...
class URLAliasService implements URLAliasServiceInterface
{
     ...
     * @param array|null $prioritizedLanguages By default taken from config, used as prioritized language codes for order of returned objects.
     *
     * @return \eZ\Publish\API\Repository\Values\Content\URLAlias[]
     */
     ...
     * @return string|bool
     */
    protected function choosePrioritizedLanguageCode(array $entries, $showAllTranslations, array $prioritizedLanguages)
    {
        foreach ($prioritizedLanguages as $prioritizedLanguageCode) {
    ...
    protected function isUrlAliasLoadable(
        ...
        foreach ($spiUrlAlias->pathData as $levelPathData) {
            ...
            foreach ($levelPathData['translations'] as $translationLanguageCode => $translation) {
                if (in_array($translationLanguageCode, $prioritizedLanguages)) {
                    continue 2;
                }
            }
     ...
     * @param null|string[] $prioritizedLanguages
     *
     * @return \eZ\Publish\API\Repository\Values\Content\URLAlias
     */
